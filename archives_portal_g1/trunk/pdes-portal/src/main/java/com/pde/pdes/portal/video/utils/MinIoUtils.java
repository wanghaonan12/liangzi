...
public class MinIoUtils {
     ...
     */
    public List<FileUploadVO> filesUpload(String bucketName, String folder, MultipartFile[] multipartFile) throws Exception {
     ...
     * @return 上传信息
     */
    public VideoUploadVO videoUpload(String bucketName, String folder, MultipartFile video) {
        // 获取源文件名
        String fileName = video.getOriginalFilename();
        String objectName = null;
        Integer fileSize = Math.toIntExact(video.getSize());
        // 拆解字符串，获取文件后缀名
        String[] split = fileName.split("\\.");
        ...
        InputStream in = null;
        VideoUploadVO videoUploadVO = null;
        try {
            ...
            String filePath = Constants.MINIO_URL_PREFIX + objectName;
            videoUploadVO = VideoUploadVO.builder().filePath(filePath).fileName(prefixName).fileSize(fileSize).build();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (in != null) {
